...
void printToFile (string line[100], int length, int x, string stringType, node **tree)
{
	int lineLength = length;
	string identName;
	int flag = 0, count = 0;
...
void funcPrintToFile (string line[100], int length, int x, string stringType, string funcName, node **tree)
{
	int lineLength = length;
	string identName;
	...
	for (auto sepWord : stringSplit)
	{
		if (funcName.compare("main") == 0)
		{
			...
			else if (flag == 3)
			{
				...
				flag = 0;
				fillStruct(fullName, x+1, "var", returnType, 0, &(*tree));
...
int countFunctions (string line[100], int length, node **tree)
{
	int lineLength = length;
	...
	for (int x = 0;x < lineLength;x++)//loops through file
	{
		for (int y = 0;y < arrLength;y++)//loops through array of prefixes
		{
			...
			if (found6 != string::npos)
			{
				...
				funcCount++;
				fillStruct(identName, x+1, "func", stringArr[y], 0, &(*tree));
				outputIdentifier(line, lineLength, identName, x+1, "func", stringArr[y]);//calls function with correct parameters
				funcPrintToFile (line, lineLength, x, stringArr[y], identName, &(*tree));//calls function with correct parameters
				x = lineLength;
			}
			...
			else if (found3 != string::npos)
			{
				...
				for (auto sepWord : stringSplit)
				{
					...
					else if (flag == 1)
					{
						...
						string name = sepWord.substr(0,semiCPos);
						identName.assign(name);
						identName = remove_spaces(identName);
						string fullName = identName + "(" + funcName + ")";
						flag = 0;
						fillStruct(fullName, x+2, "var", stringArr[y], 0, &(*tree));
...
int countVariables (string line[100], int length, node **tree)
{
	int lineLength = length;
		...
		for (int y = 0;y < arrLength;y++)
		{
			...
			if (found != string::npos) 
			{
				...
				else if (found3 == string::npos && mainFound == 1)
				{
					if (line[x].find("(", found) == string::npos)
					{
						...
						if (found != string::npos)
						{
							for (int z = 0; z < arrLength;z++)
							{
								...
								{
									varCount++;
									printToFile (line, lineLength, x, stringArr2[y], &(*tree));//calls function with correct parameters
						...
						else
						{
							for (int z = 0; z < arrLength;z++)
							{
								...
								{
									varCount++;
									printToFile (line, lineLength, x, stringArr2[y], &(*tree));
									found = line[x].find(stringArr2[z], found+1); 
								} 
							}
						}
					...
					{
						varCount++;
						funcPrintToFile (line, lineLength, x, stringArr[y], "main", &(*tree));//calls function with correct parameters
					}
				}
				else if (mainFound == 0)
				{
					...
					if (found != string::npos)
					{
						for (int z = 0; z < arrLength;z++)
						{
							...
							{
								varCount++;
								printToFile (line, lineLength, x, stringArr[y], &(*tree));//calls function with correct parameters
						...
						for (int z = 0; z < arrLength;z++)
						{
							...
							if (found != string::npos && found2 != string::npos)
							{
								...
								for (auto sepWord : stringSplit)
								{
									...
									{
										varCount++;
										printToFile(line, lineLength, x, stringArr[y], &(*tree));
										flag = 0;
									}
								}
							}
				...
				{
					varCount++;
					printToFile (line, lineLength, x, "long long", &(*tree));//calls function with correct parameters
				}
